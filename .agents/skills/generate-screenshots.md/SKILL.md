...
> 4. **Which languages does your store listing ship in?** I can write the screenshot copy in each of them so the project opens fully localized. Supported: en, es, fr, de, it, pt, ru, ja, ko, zh, ar, hi, nl, sv, da, no, fi, pl, tr, th. (Default: English only)
...
#### Localized copy (if the developer chose multiple languages)

Write the headline and subtitle for each slide in EVERY chosen language yourself. Translate the marketing intent, not the words — a good German headline is written the way a German marketer would write it, not a literal translation. Keep each translation roughly the same length as the English original so it fits the layout (long compound words in German/Dutch: prefer shorter synonyms).

Each text layer then carries a `translations` map keyed by language code, and it MUST include `"en"` as well (the editor falls back to `translations.en`):

```json
"content": "Everything at a Glance",
"translations": {
  "en": "Everything at a Glance",
  "es": "Todo de un vistazo",
  "de": "Alles auf einen Blick"
}
```

The top-level `languages` array must list every chosen code with `"en"` first, e.g. `"languages": ["en", "es", "de"]`. If the developer chose English only, omit `translations` entirely and use `"languages": ["en"]`.
...
```bash
curl -s -X POST https://api.screenshotwhale.com/api/projects/create-from-skill \
...
⚠️ For multi-language projects, add the `translations` map (see "Localized copy" above) to every text layer — same map on the same slide's layer across all devices.
...
Shipping to Google Play? You'll also need a 1024x500 feature graphic — generate
one free every day at https://screenshotwhale.com/google-play-feature-graphic-generator
```
